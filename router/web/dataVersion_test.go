...
func TestBumpDataVersionForDeletedScopes(t *testing.T) {
	...
	// 班级级作用域：只推进该班，不动全局
	bumpDataVersionForDeletedScopes(db.GetDB(), []string{"s1/g1/c1"})
	...
	bumpDataVersionForDeletedScopes(db.GetDB(), []string{"s1/g1"})
	assert.False(t, db.GetDataVersion("", "", "").Version.IsZero(), "粗作用域删除应推进全局版本")

	// 同一秒内的连续删除也必须推进版本：LatestTimestamp 只精确到秒
	require.NoError(t, db.GetDB().Where("1 = 1").Delete(&dbTable.DataVersion{}).Error)
	bumpDataVersionForDeletedScopes(db.GetDB(), []string{"s2/g2/c2"})
	first := db.GetDataVersion("s2", "g2", "c2").Version
	bumpDataVersionForDeletedScopes(db.GetDB(), []string{"s2/g2/c2"})
	second := db.GetDataVersion("s2", "g2", "c2").Version
	assert.True(t, second.After(first), "同一秒内连续删除也必须让版本严格递增")

	// 重复项与空输入不应出错（幂等）
	bumpDataVersionForDeletedScopes(db.GetDB(), []string{"s1/g1/c1", "s1/g1/c1", ""})
	bumpDataVersionForDeletedScopes(db.GetDB(), nil)
}
