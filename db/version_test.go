...
func TestGetLatestVersionNs_MissingRowIsNotNegative(t *testing.T) {
	cleanupDB(t)

	assert.Equal(t, int64(0), GetLatestVersionNs("default", "no-such-school", "2024", "1").Timestamp())
}

// 自动任务的 status 只是管理端列表用的派生缓存：客户端响应在读取时按时间重新求值，
// 不依赖该列。因此状态翻转不得推进 UpdatedAt，否则每次翻转都会推进课表版本、
// 让所有客户端多做一次无意义的重新拉取。
func TestRefreshAutorunStatusesNs_KeepsUpdatedAt(t *testing.T) {
	cleanupDB(t)
	...
	updated, err := RefreshAutorunStatusesNs("default", time.Now())
