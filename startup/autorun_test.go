...
// 测试用命名空间：saas 版所有查询都按 namespace 隔离，空 namespace 会返回空集
...
func resetCleanerState(t *testing.T) {
	...
	require.NoError(t, db.GetDB().Where("namespace = ?", testNamespace).Delete(&dbTable.AutorunRecord{}).Error)
}
...
func waitForInlineCleanup(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		rows, err := db.FetchAutorunRecordsNs(testNamespace, "")
